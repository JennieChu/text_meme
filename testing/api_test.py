...
def inbound_sms():
    ...
    if  inbound_number not in number_list:
        ...
        if "Start" ==  meme:
            msg = Message().body("Welcome to TextMeme! To make a meme, text me the meme name and the top and bottom text you want, seperated by a period.")
        else:
            # Check if meme exists, if not let user know
            if meme not in open('meme_list').read():
                msg = Message().body("Having trouble? Text START to learn how to use or Check our our available memes here: INSERT").media("http://m.memegen.com/hxg2qb.jpg")
            else:
